...
def perm(key, original_permutation, index_permutation, index, step, bound):
    print("key of the cycle: {} \nfirst permutation: {} \n".format(original_permutation[index_permutation], key))
    while key != original_permutation[index_permutation]:
        if not key in original_permutation:
            print(key)
            original_permutation.append(key.copy())
        key = move_left(key, index, step)
        index = (index+step)%bound
    print()
    return original_permutation
